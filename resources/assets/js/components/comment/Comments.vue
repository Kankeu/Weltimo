...
<template>
    <div>
        ...
        <blockquote :id="comment.idName" v-for="comment,i in comments" :key="i">
    ...
    export default{
        ...
        computed: {
            comments(){
                ...
                return this.$store.state.query.queries.find(e=>(e.name==="comment" && !e.next && e.id===this.article.id))
            },
            query(){
                ...
                if(!this.end && this.ready){
                    ...
                    this.$http.get(this.query.next).then(response=>{
                        if(response.body && typeof response.body === "object"){
                            ...
                            this.$store.dispatch("query/save",{name:'comment',next:response.body.next_page_url,id:this.article.id})
                            this.$nextTick(()=>{
